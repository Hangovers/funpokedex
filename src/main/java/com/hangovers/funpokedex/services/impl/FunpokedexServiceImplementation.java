...
public class FunpokedexServiceImplementation implements FunpokedexService {
     ...
     */
    public Publisher<Pokemon> getTranslatedPokemon(String pokemonName) {
        return getPokemon(pokemonName).publishOn(Schedulers.boundedElastic()).map(pokemon -> {
            if (pokemon.name().equalsIgnoreCase("MissingNo.") || pokemon.name().equalsIgnoreCase("Bad EGG"))
                return pokemon;
    ...
    private Mono<String> fetchYodaTranslations(TranslationRequest request) {
        return funtranslationsapiClient
                ...
                .map(funtranslationsapiResponse -> {
                ...
                .onErrorMap(t -> new HttpClientResponseException(t.getMessage(), HttpResponse.serverError()));
    }
