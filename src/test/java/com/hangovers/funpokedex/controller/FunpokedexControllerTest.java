...
/**
 * The purpose of this test class is to test overall application behaviour as if it was a real world run.
 * Embedded servers are created to allow tests to response with specific jsons and avoid loading
 * real servers and incur in issues such as rate limiting.
 * Caching is also tested.
 */
...
class FunpokedexControllerTest {
    ...
    public static void startServer() {
        ...
        embeddedServer = ApplicationContext.run(
                EmbeddedServer.class, Collections.singletonMap("pokeapi.url", "http://localhost:" + pokeapi.getPort()));
    }
    ...
    static class PokeApiMockClient {
        ...
        Optional<String> getPokemon() {
            return resourceLoader.getResourceAsStream("mewtwo.json").flatMap(inputStream -> {
                try {
